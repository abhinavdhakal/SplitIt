...
export function parseReceiptText(text) {
  ...
  // STRATEGY 1: Look for the exact Walmart pattern
  console.log("Strategy 1: Walmart format with status words");
  const walmartPattern =
    /([A-Za-z][^$\n]{2,40}?)\s+(Shopped|Unavailable|Available)\s+Qty\s+(\d+)\s+\$([0-9]+\.[0-9]{2})/gi;
  ...
  while ((match = walmartPattern.exec(cleanText)) !== null) {
    ...
    console.log("🎯 Walmart pattern match:", { name, status, qty, price });

    if (name && !/(total|tax|tip|subtotal|fee)/i.test(name)) {
      items.push({
        name: name,
        quantity: qty,
        unit_price: price,
